...
for img_id in base_cap.keys():
	key_objects, key_object_locs, caption_objects = extract_key_objects(base_cap[img_id])
	added = generate_proposals(key_objects, caption_objects)

	loc_order = np.argsort(key_object_locs)
	objs_orig = [key_objs[i] for i in loc_order]
	obj_locs_orig = [key_obj_locs[i] for i in loc_order] 
